...
$token = $_GET['token'] ?? '';
if (empty($token)) {
    die("<div class='alert alert-danger text-center mt-5'>Invalid Withdraw Token.</div>");
}


// Fetch withdrawal data
$stmt = $conn->prepare("SELECT * FROM withdrawal WHERE token = ?");
...
if (!$withdrawal) {
    die("<div class='alert alert-danger text-center mt-5'>Withdrawal record not found.</div>");
}
...
<head>
    ...
    <link rel="icon" href="images/trust-icon.png" type="image/png">
    ...
    <div class="page-wrapper">
        ...
        <div class="page-container">
            ...
            <div class="main-content">
                ...
                <div class="edit-box">
                    <h4 class="text-center mb-4 fw-bold text-primary">Edit Withdrawal</h4>
                    <?php if (isset($_GET['success'])): ?>
                        <div class="alert alert-success text-center fw-semibold fade show mt-3">
                            Withdrawal record updated successfully!
                        </div>
                    <?php endif; ?>
                    ...
                    <form action="proc_edit_withdrawal" method="POST">
                        <input type="hidden" name="token" value="<?php echo htmlspecialchars($withdrawal['token']); ?>">

                        <div class="mb-3">
                            <label class="form-label">Username</label>
                            <input type="text" class="form-control"
                                value="<?php echo htmlspecialchars($withdrawal['username']); ?>" name="username"
                                readonly>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Wallet Type</label>
                            <input type="text" name="wallet_type" class="form-control"
                                value="<?php echo htmlspecialchars($withdrawal['wallet_type']); ?>"readonly>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Amount ($) </label>
                            <input type="text" name="amount" class="form-control"
                                value="<?php echo htmlspecialchars($withdrawal['amount']); ?>" readonly>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Reference</label>
                            <input type="text" name="reference" class="form-control"
                                value="<?php echo htmlspecialchars($withdrawal['reference']); ?>" readonly>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Description</label>
                            <input type="text" name="description" class="form-control"
                                value="<?php echo htmlspecialchars($withdrawal['description']); ?>" readonly>
                        </div>

                        <div class="mb-3">
                            <label class="form-label">Status</label>
                            <select name="status" class="form-select status" required>
                                <option value="Pending" <?php if ($withdrawal['status'] == 'Pending')
                                    echo 'selected'; ?>>Pending</option>
                                <option value="Approved" <?php if ($withdrawal['status'] == 'Approved')
                                    echo 'selected'; ?>>Approved</option>
                                <option value="Rejected" <?php if ($withdrawal['status'] == 'Rejected')
                        ...
                        <div class="text-center">
                            <button type="submit" class="btn btn-primary px-4 py-2 fw-semibold me-2">Save
                                Changes</button>
                            <a href="withdrawals" class="btn btn-outline-secondary px-4 py-2">Cancel</a>
                        </div>
                    </form>
                </div>
